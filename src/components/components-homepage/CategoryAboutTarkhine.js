...
const CategoryAboutTarkhine = () => {
	...
	return (
		<div className='my-4x w-full h-full grid grid-cols-1 md:grid-cols-4 grid-rows-4 md:grid-rows-1 gap-y-4 md:gap-x-4'>
			{category.map(({ img, branch, address }, index) => (
				<div
					key={index}
					className='transition group hover:shadow-cart-shadow hover:border-Primary hover:-translate-y-2 rounded-2x border-2 border-Neutral-Gray-4 bg-white p-1 w-full h-10x md:h-13x lg:h-15x 2xl:h-16x center-items-flex flex-row md:flex-col'
				>
					<div
						className='w-1/2 md:w-full h-full md:h-3/5 xl:h-[70%] rounded-2x bg-cover'
					...
					<div className='px-2 w-1/2 md:w-full h-full md:h-2/5 xl:h-[30%] center-items text-Neutral-Gray-8'>
						<h2 className='text-center Caption-L-B lg:Caption-XL-B font-bold'>{branch}</h2>
						<p className='text-center Caption-XS-M lg:Caption-S-M 2xl:Caption-L-M'>{address}</p>
					</div>
				</div>
			))}
		</div>
